...
if (!file.exists("Files/power_consumption.zip")) {
 ...
 # write a clean data set to the directory
  write.table(hpc_subset, file="Files/power_consumption_tidy.txt", sep="|", row.names=FALSE)


# Generating Plot 1
  hist(hpc_subset$Global_active_power, main = paste("Global Active Power"), col="red", xlab="Global Active Power (kilowatts)")
  dev.copy(png, file="Files/plot1.png", width=480, height=480)
  dev.off()
